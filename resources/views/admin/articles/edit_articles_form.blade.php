...
@php
    $title='Edit & Update Blog';
    $subTitle = 'Edit & Update Blog';
                ...
                <div class="col-lg-8">
                    <div class="card mt-24">
                        ...
                        <div class="card-body p-24">
                            <form id="myForm" action="{{route('admin.update_articles',$blogDataById->id)}}" method="post" class="d-flex flex-column gap-20" enctype="multipart/form-data">
                                @csrf
                                @method('PUT')
                                <div class="row">
                                <div class="col-sm-6">
                                    <label class="form-label fw-bold text-neutral-900" for="title">Post Title: </label>
                                    <input type="text" name="title" value="{{$blogDataById->title}}" class="form-control border border-neutral-200 radius-8" id="title" placeholder="Enter Post Title">
                                </div>
                                <div class="col-sm-6">
                                    <label class="form-label fw-bold text-neutral-900" for="title">Status: </label>
                                    <select name="status" class="form-control">
                                        <option value="published" @if($blogDataById->status=='published') selected @endif >Published</option>
                                        <option value="draft" @if($blogDataById->status=='draft') selected @endif>Draft</option>
                                        <option value="archived" @if($blogDataById->status=='archived') selected @endif>Archived</option>
                                    </select>
                                </div>
                                </div>
                                <div>
                                    ...
                                    <select name="category" class="form-control border border-neutral-200 radius-8">
                                        ...
                                        <option value="{{strtolower($allcatVal->cat_name)}}" @if($blogDataById->category==strtolower($allcatVal->cat_name)) selected @endif>{{$allcatVal->cat_name}}</option>
                                        @endforeach
                                ...
                                <div>
                                    ...
                                    <input type="text" name="tags" value="{{$blogDataById->tags}}" class="form-control border border-neutral-200 radius-8" id="title" placeholder="Enter Blog Tag">
                                </div>
                                <div>
                                    ...
                                    <div class="border border-neutral-200 radius-8 overflow-hidden">
                                        <div class="height-200">
                                            ...
                                            <!-- Editor start -->
                                            <div id="editor">{!! $blogDataById->description !!}</div>
                                ...
                                <div>
                                    ...
                                    @if(!empty($blogDataById->bgimage))
                                    <img src="{{asset('storage/'.$blogDataById->bgimage)}}" style="width:100px;">
                                    @endif
